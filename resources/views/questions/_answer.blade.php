    ...
    <div class="media-body">
      <div class="media-heading mt-0 mb-1 text-secondary">
        ...
        <div class="float-right btn-toolbar">
          @if($answer->isBest())
            <button class="btn btn-outline-warning btn-sm mt-2"><i class="fa fa-check"></i> 最佳答案</button>
          @else
            @can('update', $question)
            ...
            @can('delete', $answer)
              <form action="/answers/{{ $answer->id }}" method="POST" accept-charset="UTF-8">
                {{ method_field("DELETE") }}
                {{ csrf_field() }}
                <button type="submit" class="btn btn-outline-danger btn-sm mt-2 ml-2">删除</button>
              </form>
            @endcan
          @endif
        </div>
