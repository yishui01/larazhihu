        ...
        <div class="card-body">
          ...
          <form action="/questions" method="POST" accept-charset="UTF-8">
            ...
            <div class="form-group">
              <input class="form-control" type="text" name="title" value="{{ old('title', $question->title ) }}" placeholder="请填写标题"  dusk="question-title" />
            </div>
            ...
            <div class="form-group">
              <select class="form-control" name="category_id" required dusk="question-category">
                ...
                @foreach ($categories as $value)
                  <option value="{{ $value->id }}" <?php if (old('category_id') == $value->id){
                      echo "selected";
                  } ?> >{{ $value->name }}</option>
                @endforeach
              </select>
